...
class Warp_net(nn.Module):
    def __init__(self):
        ...
        self.feature_ext = nn.Conv2d(6, channelnum, 3, padding=1)
        self.f_relu = nn.ReLU()
        torch.nn.init.xavier_uniform_(self.feature_ext.weight.data)
        torch.nn.init.constant_(self.feature_ext.bias.data, 0.0)

        self.conv0 = ResBlock(channelnum, channelnum, 3)
        self.conv0_p = nn.AvgPool2d(2, 2)
        self.conv1 = ResBlock(channelnum, channelnum, 3)
        self.conv1_p = nn.AvgPool2d(2, 2)
        self.conv2 = ResBlock(channelnum, channelnum, 3)
        self.conv3 = ResBlock(channelnum, channelnum, 3)
        self.conv4 = ResBlock(channelnum, channelnum, 3)
        self.conv5 = ResBlock(channelnum, channelnum, 3)
        ...
        # Modified - Spatial Attention Added
    ...
    def forward(self, x):
        ...
        # Modified - Spatial Attention Applied
        sa = self.spatial_attention(c3)
        c3 = c3 * sa

        c3_u = c1 + bilinearupsacling2(c3)
        c4 = self.conv4(c3_u)
        c4_u = c0 + bilinearupsacling2(c4)
        c5 = self.conv5(c4_u)
        res = self.conv6(c5)
        return res
...
def block_matching(prev_frame, curr_frame, block_size=16, search_range=7):
